...
%addpath(('D:\v lfeat-0.9.14\toolbox'));
addpath('E:\Software\vlfeat-0.9.14\vlfeat-0.9.14\toolbox')
vl_setup
...
evalType='SRER'; %'OPE','SRE','TRE'


diary(['./tmp/' evalType '.txt']);%commend record.

numSeq=length(seqs);%number of different seqs
numTrk=length(trackers);% number of different tracker methods.

%finalPath = ['./results/results_' evalType '_CVPR13/'];
finalPath = ['./results/pami15_' evalType '/pami15_' evalType];
...
for idxSeq=50:length(seqs) %by zhangzheng 2016/12/29
    %idxSeq=14;
    s = seqs{idxSeq};
...
%      end
    ...
    [subSeqs, subAnno]=splitSeqTRE(s,numSeg,rect_anno);%sample rect_anno ÊÇground truth
    
    
   % evalType='TRE';% by zhangzheng 12/05/16
    switch evalType
        case 'SRE'
            subS = subSeqs{1};
            subA = subAnno{1};
    ...
    %for idxTrk=1:numTrk %by zhangzheng 12/06/16
        idxTrk=14;
        t = trackers{idxTrk};
